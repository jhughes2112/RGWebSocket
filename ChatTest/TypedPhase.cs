		...
		namespace ChatTest
		{
			...
			public class PackedBinaryFactory : IMessageFactory
			{
				public void Serialize(IRGMessage msg, IBufferWriter<byte> writer)
				{
					...
					{
						case TPing.kTypeId:
							WriteInt(writer, ((TPing)msg).Seq);
							break;
						case TPong.kTypeId:
							WriteInt(writer, ((TPong)msg).Seq);
							break;
						case TChat.kTypeId:
							writer.Write(System.Text.Encoding.UTF8.GetBytes(((TChat)msg).Text));
							break;
						default:
							throw new InvalidOperationException($"PackedBinaryFactory cannot serialize unknown TypeId {msg.TypeId}");
					}
				}
				...
				public IRGMessage Deserialize(int typeId, ReadOnlySpan<byte> payload)
				{
					...
					{
						case TPing.kTypeId:
							return payload.Length == 4 ? new TPing() { Seq = BinaryPrimitives.ReadInt32LittleEndian(payload) } : null;
						case TPong.kTypeId:
							return payload.Length == 4 ? new TPong() { Seq = BinaryPrimitives.ReadInt32LittleEndian(payload) } : null;
						case TChat.kTypeId:
							return new TChat() { Text = System.Text.Encoding.UTF8.GetString(payload) };
						default:
							return null; // unknown type id -> ProtocolError disconnect
					}
				}
			...
			public class JsonFactory : IMessageFactory
			{
				...
				public IRGMessage Deserialize(int typeId, ReadOnlySpan<byte> payload)
				{
					try
					{
						...
						{
							case TPing.kTypeId:
								return JsonSerializer.Deserialize<TPing>(payload, kOptions);
							case TPong.kTypeId:
								return JsonSerializer.Deserialize<TPong>(payload, kOptions);
							case TChat.kTypeId:
								return JsonSerializer.Deserialize<TChat>(payload, kOptions);
							default:
								return null;
						}
					}
			...
			static public class TypedPhase
			{
				...
				static public async Task<(bool ok, long protocolErrors, string detail)> Run(int port, IMessageFactory factory, bool includeMalformed, ILogging logger)
				{
					...
					RGWebServer     server = new RGWebServer($"http://localhost:{port}/", 2, 5000, 30, mgr, logger, null, null);
					server.Start();
					try
					{
